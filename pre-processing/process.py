...
class MatrixGeneration:
    ...
    def generate_user_matrices(
        ...
        GOWALLA_7_CATEGORIES_TO_INT = {GOWALLA_7_CATEGORIES[i]: i for i in range(len(GOWALLA_7_CATEGORIES))}

        user_checkin[category_column] = np.array(
            [GOWALLA_7_CATEGORIES_TO_INT[i] for i in user_checkin["category"].tolist()]
        )
        ...
        placeids_unique_to_int = {
            placeids_unique[i]: i for i in range(len(placeids_unique))
        }

        # converter os ids dos locais para inteiro
        placeids_int = [
            placeids_unique_to_int[placeids[i]] for i in range(len(placeids))
        ]
        ...
        for j in range(1, len(datetimes)):
            ...
            if distance_matrix[local_anterior][local_atual] == 0:
                distance = int(
                    self._points_distance(
                        [lat_before, lng_before], [lat_current, lng_current]
                    )
                )
            ...
            if datetimes[atual].weekday() < 5:
                adjacency_weekday_matrix[placeids_int[anterior]][
                    placeids_int[atual]
                ] += 1
                adjacency_weekday_matrix[placeids_int[atual]][
                    placeids_int[anterior]
                ] += 1
            else:
                adjacency_weekend_matrix[placeids_int[anterior]][
                    placeids_int[atual]
                ] += 1
                adjacency_weekend_matrix[placeids_int[atual]][
                    placeids_int[anterior]
                ] += 1
        ...
        adjacency_matrix_df = user_checkin[
            ["userid", "adjacency", "category", "visited_location_ids"]
        ]
        adjacency_weekday_matrix_df = user_checkin[
            ["userid", "adjacency_weekday", "category"]
        ]
        adjacency_weekend_matrix_df = user_checkin[
            ["userid", "adjacency_weekend", "category"]
        ]
        temporal_matrix_df = user_checkin[["userid", "temporal", "category"]]
        temporal_weekday_matrix_df = user_checkin[
            ["userid", "temporal_weekday", "category"]
        ]
        temporal_weekend_matrix_df = user_checkin[
            ["userid", "temporal_weekend", "category"]
        ]
        distance_matrix_df = user_checkin[["userid", "distance", "category"]]
        duration_matrix_df = user_checkin[["userid", "duration", "category"]]

        adjacency_matrix_df.columns = [
            "user_id",
            "matrices",
            "category",
            "visited_location_ids",
    ...
    def _create_location_coocurrency_matrix(
        ...
        try:
            ...
            for user_id in users_checkins[userid_column].unique():
                ...
                cont += 1
                users_checkins_sorted = users_checkins[
                    users_checkins[userid_column] == user_id
                ].sort_values(by=[datetime_column])
                ...
                for i in range(len(locations)):
                    ...
                    for j in range(1, 6):
                        if (i - j) < 0:
                            break
                        self.LL[
                            current_location, locationid_to_int[locations[i - j]]
                        ] += 1
                    for j in range(1, 6):
                        if (i + j) > len(locations) - 1:
                            break
                        self.LL[
                            current_location, locationid_to_int[locations[j + i]]
                        ] += 1
    ...
    def _create_LT_matrix(
        self, users_checinks, locationid_column, datetime_column, locationid_to_int
    ):
        ...
        for i in range(len(locations)):
            current_location = locationid_to_int[locations[i]]
            if datetimes[i].weekday() >= 5: # Saturday
    ...
    def generate_pattern_matrices(
        ...
        # shuffle
        users_checkin = users_checkin.sample(frac=1, random_state=1).reset_index(
            drop=True
        )
        print("\n", users_checkin)
        ...
        locationid_to_int = {
            unique_locationsids[i]: i for i in range(len(unique_locationsids))
        }
        keys = list(locationid_to_int.keys())
        values = list(locationid_to_int.values())
        self._create_LT_matrix(
            users_checkin, locationid_column, datetime_column, locationid_to_int
        )
        print("\nterminou LT")

        lt = pd.DataFrame(self.LT, columns=[str(i) for i in range(self.LT.shape[1])])

        lt.to_csv(location_time_pmi_matrix_filename, index=False)

        pd.DataFrame({"locationid": keys, "int": values}).to_csv(
            int_to_locationid_filename, index=False
        )
    ...
    def remove_gps_pois_that_dont_have_categories(
        self, categories, adjacency_matrix, features_matrix
    ):
        ...
        adjacency_matrix = adjacency_matrix[
            indexes_filtered_pois[:, None], indexes_filtered_pois
        ]
    ...
    def _points_distance(self, point_0, point_1):
